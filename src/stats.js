;(async function() {
  ...
  const membersWithRepositories = members.filter(
    member => member.repositories.length > 0,
  )
  stats.membersWithRepositories = membersWithRepositories.length

  const repositories = members.flatMap(member => member.repositories)
  ...
  const repositoriesOwnedByMembers = membersWithRepositories
    .map(member => {
      member.repositories = member.repositories.filter(
        repository => repository.owner.login === member.login,
      )
      return member
    })
  ...
  stats.topMemberRepositories = repositoriesOwnedByMembers
    .reduce((acc, next) => {
      acc.push([
        `${next.name} (${next.owner.login})`,
        next.stargazers.totalCount,
      ])
      return acc
    }, [])
    .sort((a, b) => b[1] - a[1])
  ...
  stats.topOrganizationRepositories = organizationRepositories
    ...
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([{ name, url, description }, count]) => ({
      name,
      description,
      count,
      url,
  ...
  stats.topRepositories = organizationRepositories
    .concat(repositoriesOwnedByMembers)
    .reduce((acc, next) => {
      acc.push([{ ...next }, next.stargazers.totalCount])
      return acc
    }, [])
  ...
  if (generateFile) {
    try {
      fs.writeFileSync(
        path.join(__dirname, dataFolder, statsFile),
        JSON.stringify(stats, undefined, 2),
      )
      console.log(
        chalk`{green Stats file for ${stats.organization} has been generated in /data/stats.json 📄}`,
      )
    } catch (e) {
      console.error(
        'Error while generating stats file',
    ...
    console.log(chalk`
      ...
      Organization top languages:\r\n{blue ${stats.topPrimaryLanguages
        .map(({ language, count }) => `\t- ${language}: ${count}`)
        .join('\r\n')}}
      Top contributors:\r\n{blue ${stats.topContributors
        .map(
          ({ login, totalContributions }) =>
            `\t- ${login}: ${totalContributions}`,
        )
        .join('\r\n')}}
      Organization top repositories:\r\n{blue ${stats.topOrganizationRepositories
        .map(({ name, count }) => `\t- ${name}: ${count} ⭐️`)
        .join('\r\n')}}
      Top repositories:\r\n{blue ${stats.topRepositories
        .map(({ name, count }) => `\t- ${name}: ${count} ⭐️`)
        .join('\r\n')}}
      ${stats.organization} members repositories: {blue ${
      stats.totalRepositories
    }}
      Top languages:\r\n{blue ${stats.topLanguages
        .map(({ language, count }) => `\t- ${language}: ${count}`)
        .join('\r\n')}}
      Top ${
        stats.organization
      } members repositories:\r\n{blue ${stats.topMemberRepositories
      .map(({ repo, count }) => `\t- ${repo}: ${count} ⭐️`)
      .join('\r\n')}}`)
  }
})()
